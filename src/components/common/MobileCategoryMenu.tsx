...
export default function MobileCategoryMenu({
  ...
  const categoryMenuContent = (
    <div className="sm:hidden">
      <div className="fixed top-14 inset-x-0 bottom-0 bg-black/20 z-30" onClick={onClose} />
      <div
        className={`fixed top-14 inset-x-0 w-full sm:hidden bg-white/90 backdrop-blur-lg flex flex-col items-center gap-2.5 p-4 z-40 ${className}`}
      >
        {items.map((item) => {
